...
void setup() {
  ...
  while (file.available()) {
    ...
    if (line.startsWith("ssid:")) {
      ...
      placa = line.substring(6);
    }
  }
  file.close(); // Removido o caractere 'A' solto
...
void loop() {
  ...
  if (Serial.available() > 0) { // Verifica se há dados disponíveis para leitura no Serial Monitor
    ...
    if (input == "gps") {
      ...
      // Se a entrada for "sdenvi", enviar o conteúdo do arquivo do cartão SD
      verificarEnvioCartaoSD();
    } else if (input == "sdadd") {
  ...
  if (gps.location.isValid()) {
    ...
    char latStr[15];
    char lonStr[15];
    dtostrf(latitude, 10, 6, latStr);
    dtostrf(longitude, 10, 6, lonStr);
    ...
    if (precision <= 7) {
      Serial.println("Precisão aceitável: " + String(precision) + "m");
      saveToSD(latStr, lonStr, date_time);
    } else {
      Serial.println("Precisão NÃO aceitável: " + String(precision) + "m");
    }

    delay(5000);
  }

  // Verificar se está conectado e enviar dados se necessário
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("Dados enviados ao Google");
    verificarEnvioCartaoSD();
  }

  delay(40000); // Ajuste este delay conforme necessário
}
...
void checkAndReconnectWiFi() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Não conectado ao WiFi. Verificando redes disponíveis...");
    
    // Inicia o scan de redes Wi-Fi
    ...
    if (n == 0) {
      ...
      // Itera sobre as redes encontradas
      for (int i = 0; i < n; ++i) {
        String ssid = WiFi.SSID(i);
        Serial.print("Rede encontrada: ");
        Serial.println(ssid);
        
        // Verifica se o SSID está na lista de credenciais
        for (const auto& creds : wifiCredentialsList) {
          if (ssid.equals(creds.ssid)) {
            Serial.print("Tentando conectar-se à rede: ");
            Serial.println(ssid);
            WiFi.begin(creds.ssid.c_str(), creds.password.c_str());
            
            // Aguarda até 10 segundos para tentar a conexão
            ...
            if (WiFi.status() == WL_CONNECTED) {
              Serial.println("Conectado à rede WiFi: " + ssid);
              return; // Sai da função após uma conexão bem-sucedida
            } else {
              Serial.println("Falha na conexão à rede: " + ssid);
            }
          }
        }
      }
      Serial.println("Nenhuma das redes conhecidas está disponível.");
    }
    
    // Libera a memória utilizada pelo scan
    WiFi.scanDelete();
  }
}
...
void verificarEnvioCartaoSD() {
  ...
  dataFile = SD_MMC.open("/data.txt", FILE_WRITE);
  if (dataFile) {
    Serial.println("Salvando conteudo dentro do arquivo ");
...
void smartdelay_gps(unsigned long ms) { // Removido 'static'
